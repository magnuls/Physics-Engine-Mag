...
TEST(OBBTest, AxisAlignedMissAndHit) {
    ...
    EXPECT_NEAR(miss.m_normal.GetX(), 1.0f, 1e-3f);  // points a to b along +x
...
// changes the result. That is what makes it an OBB and not an AABB.
TEST(OBBTest, RotatedReachChangesOverlap) {
    ...
    // Pulled in to x=2.3 the diagonal corner pokes into a and hits, whereas an
    // axis aligned box of reach 1 at x=2.3 would still miss.
...
TEST(OBBTest, SphereClosestPoint) {
    ...
    EXPECT_NEAR(r1.m_normal.GetX(), 1.0f, 1e-3f);  // box to sphere
    ...
    // Rotated box: its +x corner sits at about 1.414 on x, a sphere at x=1.6
...
// Swapped ordering flips the normal, preserving the A to B convention.
...
TEST(OBBTest, PlaneProjectedRadius) {
    ...
    EXPECT_NEAR(r1.m_normal.GetY(), -1.0f, 1e-3f);  // box to plane, down
...
// Simulation with droppable OBBs

// An oriented box falling onto the static floor bounces with velocity reflected.
...
// A dynamic box dropped onto a static box bounces off it. Exercises OBB vs OBB
// through broadphase, narrowphase, and response.
