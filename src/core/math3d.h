...
class Vector {
   public:
    ...
    inline T Dot(const Vector<T, D>& r) const {
        T result = T(0);
        for (unsigned int i = 0; i < D; i++) result += (*this)[i] * r[i];
    ...
    inline T Max() const {
        ...
        for (int i = 0; i < D; i++)
            if ((*this)[i] > maxVal) maxVal = (*this)[i];
    ...
    inline Vector<T, D> operator+(const Vector<T, D>& r) const {
        Vector<T, D> result;
        for (unsigned int i = 0; i < D; i++) result[i] = values[i] + r[i];
    ...
    inline Vector<T, D> operator*(const T& r) const {
        Vector<T, D> result;
        for (unsigned int i = 0; i < D; i++) result[i] = values[i] * r;
    ...
    inline Vector<T, D>& operator+=(const Vector<T, D>& r) {
        for (unsigned int i = 0; i < D; i++) (*this)[i] = values[i] + r[i];
    ...
    inline Vector<T, D>& operator*=(const T& r) {
        for (unsigned int i = 0; i < D; i++) (*this)[i] = values[i] * r;
   ...
   public:
    ...
    inline Matrix<T, D> Inverse() const {
        ...
        for (i = 0; i < D - 1; i++) {
            ...
            if (pivotsize == 0) {
                // if (singExc)
                //	throw ::Imath::SingMatrixExc ("Cannot invert singular
                // matrix.");
    ...
    inline Vector<T, D> Transform(const Vector<T, D>& r) const {
        ...
        for (unsigned int i = 0; i < D; i++) {
            ret[i] = 0;
            for (unsigned int j = 0; j < D; j++) ret[i] += m[j][i] * r[j];
        }
    ...
    inline Vector<T, D - 1> Transform(const Vector<T, D - 1>& r) const {
        ...
        for (int i = 0; i < D - 1; i++) ret[i] = ret2[i];
   ...
   public:
    ...
    Matrix4(const Matrix<T, D>& r) {
        if (D < 4) {
            ...
            for (unsigned int i = 0; i < D; i++)
                for (unsigned int j = 0; j < D; j++) (*this)[i][j] = r[i][j];
        } else {
            for (unsigned int i = 0; i < 4; i++)
                for (unsigned int j = 0; j < 4; j++) (*this)[i][j] = r[i][j];
        }
    }
    ...
    inline Matrix4<T> InitRotationFromVectors(const Vector3<T>& n,
                                              const Vector3<T>& v,
                                              const Vector3<T>& u) {
        (*this)[0][0] = u.GetX();
        (*this)[1][0] = u.GetY();
    ...
    inline Matrix4<T> InitRotationFromDirection(const Vector3<T>& forward,
                                                const Vector3<T>& up) {
    ...
    inline Matrix4<T> InitOrthographic(T left, T right, T bottom, T top, T near,
                                       T far) {
    ...
    Matrix3(const Matrix<T, D>& r) {
        if (D < 3) {
            ...
            for (unsigned int i = 0; i < 3; i++)
                for (unsigned int j = 0; j < 3; j++) (*this)[i][j] = r[i][j];
        }
    }
};
...
class Vector3f : public Vector3<float> {
   public:
    ...
    inline Vector3f operator+(const Vector3f& r) const {
        return Vector3f(GetX() + r.GetX(), GetY() + r.GetY(),
                        GetZ() + r.GetZ());
    }
    inline Vector3f operator-(const Vector3f& r) const {
        return Vector3f(GetX() - r.GetX(), GetY() - r.GetY(),
...
class Quaternion : public Vector4<float> {
   public:
    ...
    inline Quaternion NLerp(const Quaternion& r, float lerpFactor,
    ...
    inline Quaternion SLerp(const Quaternion& r, float lerpFactor,
                            bool shortestPath) const {
    ...
    inline Matrix4f ToRotationMatrix() const {
        Vector3f forward =
            Vector3f(2.0f * (GetX() * GetZ() - GetW() * GetY()),
                     2.0f * (GetY() * GetZ() + GetW() * GetX()),
                     1.0f - 2.0f * (GetX() * GetX() + GetY() * GetY()));
        Vector3f up =
            Vector3f(2.0f * (GetX() * GetY() + GetW() * GetZ()),
                     1.0f - 2.0f * (GetX() * GetX() + GetZ() * GetZ()),
                     2.0f * (GetY() * GetZ() - GetW() * GetX()));
        Vector3f right =
            Vector3f(1.0f - 2.0f * (GetY() * GetY() + GetZ() * GetZ()),
    ...
    inline Quaternion operator*(const Quaternion& r) const {
        const float _w = (GetW() * r.GetW()) - (GetX() * r.GetX()) -
                         (GetY() * r.GetY()) - (GetZ() * r.GetZ());
        const float _x = (GetX() * r.GetW()) + (GetW() * r.GetX()) +
                         (GetY() * r.GetZ()) - (GetZ() * r.GetY());
        const float _y = (GetY() * r.GetW()) + (GetW() * r.GetY()) +
                         (GetZ() * r.GetX()) - (GetX() * r.GetZ());
        const float _z = (GetZ() * r.GetW()) + (GetW() * r.GetZ()) +
    ...
    inline Quaternion operator*(const Vector3<float>& v) const {
        ...
        const float _x =
            (GetW() * v.GetX()) + (GetY() * v.GetZ()) - (GetZ() * v.GetY());
        const float _y =
