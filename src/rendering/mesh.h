...
class IndexedModel {
   public:
    ...
    inline void AddTangent(float x, float y, float z) {
        AddTangent(Vector3f(x, y, z));
    }

    void AddFace(unsigned int vertIndex0, unsigned int vertIndex1,
    ...
    inline const std::vector<Vector2f>& GetTexCoords() const {
        return m_texCoords;
    }
    inline const std::vector<Vector3f>& GetNormals() const { return m_normals; }
    inline const std::vector<Vector3f>& GetTangents() const {
        return m_tangents;
    }
...
class MeshData : public ReferenceCounter {
   ...
   private:
    ...
    void operator=(MeshData& other) = delete;
...
//
//	void AddVertex(const Vector3f& vert);
////	inline void AddVertex(float x, float y, float z) { AddVertex(Vector3f(x,
/// y, z)); }
//
//	void AddTexCoord(const Vector2f& texCoord);
////	inline void AddTexCoord(float x, float y) { AddTexCoord(Vector2f(x, y));
///}
//
//	void AddFace(unsigned int vertIndex0, unsigned int vertIndex1, unsigned
// int vertIndex2);
//
